function fig_render = render_localization (app)
    % 
    %
    ...
    fig_name = strcat("Render Preview : ", app.data.file.name);
    fig_render = findall(0, 'Type', 'figure', 'Name', fig_name);


    if isempty(fig_render)

        fig_render = figure('Name', fig_name, 'NumberTitle', 'off', 'Tag', "figure_render", "DeleteFcn", @delete_fig);
        ax = axes (fig_render);
        ...
        image_handle = imshow(app.data.render.images.image, 'parent', ax);
        axis( ax, 'image' );
        ...
        app.data.render.images = generate_preview (app.data.render.param);
        img = app.data.render.images.image;
    ...
    function delete_fig (~, ~)
        idx = find( cellfun(@(x) isequal(x, fig_render), app.children_apps) );
    ...
    function keyPressed (~, evt)
        ...
        param = app.data.render.param;

        switch evt.Key

            case 'leftarrow'    % z - 1*(voxel depth)
                param.posZ = max(min(app.data.prop.xyz(:,3)), param.posZ - param.voxel_depth);
            ...
            case 'rightarrow'   % z + 1*(voxel depth)
                param.posZ = min(max(app.data.prop.xyz(:,3)), param.posZ + param.voxel_depth);
                param.posZ = round( param.posZ * 10 ) / 10; % allow 0.1 nm precision on Z position
                img = find_image ();
                set(image_handle, 'cdata', img);
                auto_lut (image_handle);
    ...
    function config_calibration (~, ~)
        ...
        if (RIMF ~= cali.RIMF)
            ...
            app.data.render.roi(:, 3) = app.data.render.roi(:, 3) / cali.RIMF * RIMF;
            for i = 1 : length(app.data.render.images)
                z_new = app.data.render.images(i).param.posZ / cali.RIMF * RIMF;
                z_new = round( z_new * 10 ) / 10;
                app.data.render.images(i).posZ = z_new;
            end
            cali.RIMF = RIMF;
        end
        ...
        app.data.calibration = cali;
        app.data.prop.xyz = xyz;
        app.data.render.param = param;
    end
    ...
    function config_channel (~, ~)
        ...
        dcr_all = app.data.channel.dcr(~isnan(app.data.channel.dcr));
        ...
        hold on;
        app.data.channel.cut1 = max(0, ax_dcr.XLim(1)) + 0.01;
        app.data.channel.cut2 = min(1, ax_dcr.XLim(2)) - 0.01;
        app.data.channel.doCh3 = false;
        app.data.channel.RGB = [1, 2, 3]; % default LUT is RGB for the 1st, 2nd and 3rd channels
        cut1_line = images.roi.Rectangle(ax_dcr, 'Position', [0; 0; app.data.channel.cut1; ax_dcr.YLim(2)]', ...
            'Color', 'r', 'LineWidth', 0.1, 'FaceAlpha', 0.2, 'InteractionsAllowed', 'all', ...
            'Deletable', 0, 'ContextMenu', cm_dcr);

        cut2_line = images.roi.Rectangle(ax_dcr, 'Position', [app.data.channel.cut2; 0; 1-app.data.channel.cut2; ax_dcr.YLim(2)]', ...
        ...
        if model2.Converged && model2.AIC < model1.AIC && model2.BIC < model1.BIC && range(model2.ComponentProportion) < 0.6 % probably double channel data   
            ...
            app.data.channel.cut1 = find(ysum1 >= half_sum, 1, 'first');
            ysum2 = arrayfun(@(i) sum(y(id_c+1:id_c+i)), 1:101-id_c );
            app.data.channel.cut2 = find(ysum2 >= half_sum, 1, 'first');
            app.data.channel.cut1 = (id_c - app.data.channel.cut1) / 100;
            app.data.channel.cut2 = (id_c + app.data.channel.cut2) / 100;
            plot(x, y, 'r-');
            set(cut1_line, 'Position', [0; 0; app.data.channel.cut1; ax_dcr.YLim(2)]');
            set(cut2_line, 'Position', [app.data.channel.cut2; 0; 1-app.data.channel.cut2; ax_dcr.YLim(2)]');
        else
        ...
        function channel_setup (~, ~)
            ...
            do_ch3 = uicontrol('Parent',d, 'Style','checkbox', 'Position',[10 45 200 15],...
                   'String', 'keep middle portion in 3rd channel', 'Value', app.data.channel.doCh3 );
            uicontrol('Parent',d, 'Position',[100 10 55 25],'String','OK', 'Callback', @finish);

            uiwait(d);
            function finish (~, ~)
                app.data.channel.cut1 = str2double(value1.String);
                app.data.channel.cut2 = str2double(value2.String);
                set(cut1_line, 'Color', color1.String{color1.Value}, 'Position', [0; 0; app.data.channel.cut1; ax_dcr.YLim(2)]');
                set(cut2_line, 'Color', color2.String{color2.Value}, 'Position', [app.data.channel.cut2; 0; 1-app.data.channel.cut2; ax_dcr.YLim(2)]');
                app.data.channel.doCh3 = do_ch3.Value;
                %delete(d);
            end
        end
        ...
        function apply_channel ( on_trace )
            app.data.channel.do_trace = on_trace;
            app.data.channel.doChannel = true;
            app.data.channel.cut1 = cut1_line.Position(1) + cut1_line.Position(3);
            app.data.channel.cut2 = cut2_line.Position(1);
            if app.data.channel.doCh3
                app.numC = 3;
            else
                app.numC = 2;
            end
            app.data.channel.RGB(1) = find(cut1_line.Color);
            app.data.channel.RGB(2) = find(cut2_line.Color);
            img = find_image ();
            set(image_handle, 'cdata', img);
            auto_lut (image_handle);
        end
    ...
    function apply_gaussian (~, ~)
        ...
        sigma = app.data.calibration.sigma;
        if sigma(1) == 0
            return;
        end

        app.data.render.param.do_smooth = ~app.data.render.param.do_smooth;
    ...
    function create_ortho_views (~, ~)
        ...
        x = app.data.prop.xyz(:, 1); y = app.data.prop.xyz(:, 2); z = app.data.prop.xyz(:, 3);
        xedge = min(x) : app.data.calibration.pixel_size : max(x) + app.data.calibration.pixel_size;
        yedge = min(y) : app.data.calibration.pixel_size : max(y) + app.data.calibration.pixel_size;
        zedge = min(z) : app.data.calibration.voxel_depth : max(z) + app.data.calibration.voxel_depth;
        numX = length(xedge) - 1; numY = length(yedge) - 1 ; numZ = length(zedge) - 1;

        if app.data.channel.doChannel
        ...
        if app.doSmooth && app.data.calibration.sigma(1) > 0
            sigmaXY = app.data.calibration.sigma(1) / app.data.calibration.pixel_size;
        ...
        if app.doSmooth && app.data.calibration.sigma(2) > 0
            sigmaZ = app.data.calibration.sigma(2) / app.data.calibration.voxel_depth;
    ...
    function draw_roi (~, ~)
        roi = app.roi;
        roi(:, 3) = roi(:, 3) - min(app.data.prop.xyz(:, 3)); % ROI Z coordinate to figure coordinates minZ : maxZ to 0 : rangeZ
        px = app.data.calibration.pixel_size; vx = app.data.calibration.voxel_depth;
        if roi(1, 1) == min( app.data.prop.xyz(:, 1) )
            xMin = range(ax.YLim) * 0.05 + ax.YLim(1); 
        else
            xMin = roi(1, 1) / px;
        end

        if roi(1, 2) == min( app.data.prop.xyz(:, 2) )
            yMin = range(ax.XLim) * 0.05 + ax.XLim(1); 
        else
            yMin = roi(1, 2) / px;
        end

        if roi(2, 1) == max( app.data.prop.xyz(:, 1) )
            xMax = range(ax.YLim) * 0.95 + ax.YLim(1); 
        else
            xMax = roi(2, 1) / px;
        end

        if roi(2, 2) == max( app.data.prop.xyz(:, 2) )
        ...
        if app.nDim == 2     % 2D case
            ...
            if roi(1, 3) == 0   %min( app.data.prop.xyz(:, 3) )
                zMin = ZLim * 0.25 + 0.5; 
            else
                zMin = roi(1, 3) / vx;
            end
    
            if roi(2, 3) == range( app.data.prop.xyz(:, 3) )
                zMax = ZLim * 0.75 + 0.5; 
            else
                zMax = roi(2, 3) / vx;
            end
        ...
        function remove_roi (~, ~)
            delete(roi_xy);
            app.roi = [min(app.data.prop.xyz); max(app.data.prop.xyz)];
        end

        function apply_roi (~, ~)
            app.roi = [xMin*px, yMin*px, zMin*vx; xMax*px, yMax*px, zMax*vx];
            app.roi(:, 3) = app.roi(:, 3) + min(app.data.prop.xyz(:, 3));   % apply the z offset
            delete(roi_xy);
        end
    ...
    function export_to_tiff (~, ~)
        ...
        xedge = app.roi(1, 1) : app.data.calibration.pixel_size  : app.roi(2, 1) + app.data.calibration.pixel_size;
        yedge = app.roi(1, 2) : app.data.calibration.pixel_size  : app.roi(2, 2) + app.data.calibration.pixel_size;
        zedge = app.roi(1, 3) : app.data.calibration.voxel_depth : app.roi(2, 3) + app.data.calibration.voxel_depth;
        ...
        if app.data.channel.doChannel
            ...
            for c = 1 : 3
                ...
                success = render_to_tiff_file (app.data.prop.xyz, xedge, yedge, zedge, pathSave, ch_idx(:, c))  || success;
            end
        else    % no channel separation
            pathSave = fullfile(tif_dir, tif_name);
            success = render_to_tiff_file (app.data.prop.xyz, xedge, yedge, zedge, pathSave);
        end
...
%% member functions
    function load_data ()
        ...
        if ~isfield(app.data, 'calibration')
            app.data.calibration = struct;
        end
        if ~isfield(app.data.calibration, 'sigma') || isempty( app.data.calibration.sigma )
            app.data.calibration.sigma = compute_Loc_precision( app.data.prop.xyz, app.data.prop.nDim, app.data.prop.num_loc_per_trace, app.data.prop.trace_idx );
        end
        % prepare coordinate data, convert to nanometer, apply z scaling, and re-zero
        if ~isfield(app.data.calibration, 'RIMF')
            app.data.calibration.RIMF = 0.67;   % default z scale is 0.67
        end


        % init render ROI, and bit depth
        app.data.render.roi = [min(app.data.prop.xyz); max(app.data.prop.xyz)];
        app.data.render.bitDepth = 16;

        % init render image parameters:
        app.data.render.param.do_smooth = false;
        app.data.render.param.do_channel = false;
        app.data.render.param.posZ = 0;
        

        % read dcr, plot histogram, get initial value of separation
        app.data.channel.dcr = app.data.file.raw_data.dcr; % load all iteration
        app.data.channel.dcr_trace = get_dcr_trace_mean (app.data.channel.dcr, app.data.prop.trace_idx);
        %app.data.channel.doChannel = false;
        app.data.channel.do_trace = false;
        app.data.channel.numC = 1;
        app.data.channel.cut1 = 1;
        app.data.channel.cut2 = 1;
        app.data.channel.RGB = [1, 2, 3];
        

        % generate preview figure
        pixel_size = min(range(app.data.prop.xyz(:, 1:2))) * 2 / max(groot().ScreenSize(3:4));
        app.data.calibration.pixel_size = pixel_size;
        
        if app.data.prop.nDim == 3
            app.data.calibration.voxel_depth = pixel_size;
        else
            app.data.calibration.voxel_depth = 0;
        end
        app.data.render.param.pixel_size = app.data.calibration.pixel_size;
        app.data.render.param.voxel_depth = app.data.calibration.voxel_depth;
        app.data.render.param.sigma_xy = app.data.calibration.sigma(1);
        app.data.render.param.sigma_z = app.data.calibration.sigma(2);


        %app.data.render = struct;
        %app.data.render.posZ = 0;
        app.data.render.images = generate_preview (app.data.render.param);

        % app.data.render.images = struct("pixel_size", app.data.calibration.pixel_size, "voxel_depth", app.data.calibration.voxel_depth, "posZ", 0, ...
        %     "smoothed", false, "sigmaXY", app.data.calibration.sigma(1), "sigmaZ", app.data.calibration.sigma(2), ...
        %     "c", app.data.channel.numC, "channelTrace", app.data.channel.do_trace, "cut1", app.data.channel.cut1, "cut2", app.data.channel.cut2, "RGB", app.data.channel.RGB, ...
    ...
    function img = find_image ( param )
        ...
        % if nargin < 2
        %     do_smooth = app.data.render.do_smooth;
        % end
        % if nargin < 1
        %     posZ = app.data.render.posZ;
        % end


        param.posZ = round( param.posZ * 10 ) / 10; % allow 0.1 nm precision on Z position
        images = app.data.render.images;
        
        exist = arrayfun(@(x) isequaln(x, param), [images.param]);

        % exist = ...
        %     [images.z] == posZn & ...
        %     [images.pixel_size] == app.data.calibration.pixel_size & ...
        %     [images.voxel_depth] == app.data.calibration.voxel_depth & ...
        %     [images.smoothed] == do_smooth & ...
        %     [images.sigmaXY] == app.data.calibration.sigma(1) & ...
        %     [images.sigmaZ] == app.data.calibration.sigma(2) & ...
        %     [images.c] == app.numC & ...
        %     [images.channelTrace] == app.data.channel.do_trace & ...
        %     [images.cut1] == app.data.channel.cut1 & ...
        %     [images.cut2] == app.data.channel.cut2 & ...
        %     cellfun(@(imageRGB) isequaln(imageRGB, app.data.channel.RGB), {images.RGB});
        ...
        if any(exist)
            ...
            app.data.render.images(end+1) = generate_preview ( param );
            img = app.data.render.images(end).image;
            % app.data.render(end+1) = struct( ...
            %     "pixel_size", app.data.calibration.pixel_size, "voxel_depth", app.data.calibration.voxel_depth, "z", posZn, ...
            %     "smoothed", do_smooth, "sigmaXY", app.data.calibration.sigma(1), "sigmaZ", app.data.calibration.sigma(2), ...
            %     "c", app.numC, "channelTrace", app.data.channel.do_trace, "cut1", app.data.channel.cut1, "cut2", app.data.channel.cut2, "RGB", app.data.channel.RGB, ...
            %     "image", img);
        end
    ...
    %     end
    % 
    %     if app.data.render.param.do_channel
    %         ch_idx = getChannelIdx (true);
    %         xedge = min(app.data.prop.xyz(:,1)) : app.data.calibration.pixel_size : max(app.data.prop.xyz(:,1)) + app.data.calibration.pixel_size;
    %         yedge = min(app.data.prop.xyz(:,2)) : app.data.calibration.pixel_size : max(app.data.prop.xyz(:,2)) + app.data.calibration.pixel_size;
    %         counts = zeros(length(xedge)-1, length(yedge)-1, 3);
    %         for c = 1 : 3
    %             if all(ch_idx(:, c)==0)
    %                 continue;
    %             end
    %             counts(:, :, c) = get2Dhist (app.data.prop.xyz, posZ, app.data.calibration.pixel_size, ...
    %                 app.data.calibration.voxel_depth, do_smooth, ch_idx(:, c));
    %         end
    %     else
    %         counts = get2Dhist (app.data.prop.xyz, posZ, app.data.calibration.pixel_size, app.data.calibration.voxel_depth, do_smooth);
    %     end
    % 
    % end

    function ch_idx = getChannelIdx ( apply_RGB )
        if app.data.channel.do_trace
            dcr = app.data.channel.dcr_trace;
        else
            dcr = app.data.channel.dcr(:, end);
        end
        
        ch_idx = false(length(dcr), 3);
        if apply_RGB
            ch_idx(:, app.data.channel.RGB(1)) = dcr >= 0 & dcr < app.data.channel.cut1;   %R
            ch_idx(:, app.data.channel.RGB(2)) = dcr >= app.data.channel.cut2 & dcr <= 1;  %G 
        else
            ch_idx(:, 1) = dcr >= 0 & dcr < app.data.channel.cut1;   % 1st channel
            ch_idx(:, 2) = dcr >= app.data.channel.cut2 & dcr <= 1;  % 1st channel
        end
        if app.data.channel.doCh3
            ch_idx(:, 3) = dcr >= app.data.channel.cut1 & dcr < app.data.channel.cut2;  %B
        end
    end
    ...
    function counts = get2Dhist (xyz, zPos, pixel_size, voxel_depth, do_smooth, idx)
        ...
        if ~do_smooth       % no Gaussian blur, just compute 2D histogram
            if app.data.prop.nDim == 3 && voxel_depth ~= 0 % 3D case, compute hist 2D from voxel depth defined 3D volume
            ...
            if app.nDim == 2 || sigma(2) == 0 || zFilterSize < 1.0 % 2D case
                ...
                %counts_3d = get3Dhist (app.data.prop.xyz, xedge, yedge, zedge);
                ...
                for i = 1 : numZ
                    ...
                    if ismember(i, slice_save)
                        app.data.render(end+1) = struct( ...
                        "pixel_size", pixel_size, "voxel_depth", voxel_depth, "z", posZn, ...
                        "smoothed", true, "sigmaXY", app.data.calibration.sigma(1), "sigmaZ", app.data.calibration.sigma(2), ...
                        "c", app.numC, "channelTrace", app.data.channel.do_trace, "cut1", app.data.channel.cut1, "cut2", app.data.channel.cut2, "RGB", app.data.channel.RGB, ...
        ...
        elseif maxCount <= 65535
            ...
            app.data.render.bitDepth = 32;    % !!! may not work with ImageJ !!!
    ...
    function success = render_to_tiff_file (xyz, xedge, yedge, zedge, save_path, channel_idx)
        ...
        tagstruct.XResolution = 1e7 / app.data.calibration.pixel_size;
        tagstruct.YResolution = 1e7 / app.data.calibration.pixel_size;
        ...
        tagstruct.Software = ['MATLAB ', version('-release'), ' (voxle depth: ', num2str(app.data.calibration.voxel_depth), ' nm)']; % use software field to record voxel depth
    ...
    function [scale_bar, cal_box] = make_annotation ()
        ...
        axes_width_nm = range( app.data.prop.xyz(:, 2) );
