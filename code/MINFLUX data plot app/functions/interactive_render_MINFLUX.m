function interactive_render_MINFLUX (app)
    % 
    %
    % Ziqiang Huang: <ziqiang.huang@embl.de>
    % Last update: 2025.02.05
    
    %if isempty(app) || ~isempty(app.data)
     %   return;
    %end
    param = struct;
    ...
    param.data = app.data;
    param.file = struct;
    param.file.name = app.file;
    param.file.folder = app.path;
...
%% member functions
    ...
    %         dcr = itr.dcr;
    %     else
    %         if isfield(data, 'loc_x')
    %             loc = [data.loc_x, data.loc_y, data.loc_z];
    %         else
    %             loc = data.loc;
    %         end
    %         dcr = data.dcr;
